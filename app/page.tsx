      ...
      <div className={`flex h-screen ${darkMode ? "bg-gray-900" : "bg-gray-100"}`}>
        ...
        `}>
          ...
          <nav className="flex-1 px-3 py-4 space-y-1">
            <button onClick={() => { setActiveTab("chat"); setSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${activeTab === "chat" ? "bg-purple-100 dark:bg-purple-600/20 text-purple-600 dark:text-purple-300" : darkMode ? "text-gray-400 hover:text-white hover:bg-gray-700" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}>
              <span className="text-lg">💬</span><span>{t.navChat}</span>
            </button>
            <button onClick={() => { setActiveTab("tickets"); setSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${activeTab === "tickets" ? "bg-purple-100 dark:bg-purple-600/20 text-purple-600 dark:text-purple-300" : darkMode ? "text-gray-400 hover:text-white hover:bg-gray-700" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}>
              <span className="text-lg">🎫</span><span>{t.navTickets}</span>
            </button>
            <button onClick={() => { setActiveTab("settings"); setSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${activeTab === "settings" ? "bg-purple-100 dark:bg-purple-600/20 text-purple-600 dark:text-purple-300" : darkMode ? "text-gray-400 hover:text-white hover:bg-gray-700" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {messages.map((m, i) => (
                  <div key={i} className={`flex gap-2 ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                    ...
                    <div className={`max-w-[75%] px-3 py-2 rounded-lg text-sm ${m.role === "user" ? "bg-purple-600 text-white" : darkMode ? "bg-gray-700 text-gray-100" : "bg-gray-200 text-gray-800"}`}>
                      {m.content}
                    </div>
                    {m.role === "user" && (
                      <div className="w-7 h-7 rounded-lg bg-gray-200 dark:bg-gray-700 flex items-center justify-center flex-shrink-0">
                        <span className="text-gray-500 dark:text-gray-400 text-[10px] font-bold">U</span>
                      </div>
                    )}
                  </div>
                ))}
              ...
              <div className="px-4 pb-2 flex gap-2 flex-wrap">
                {t.suggestions.map((s: string) => (
                  <button key={s} onClick={() => sendMessage(s)} className={`text-xs px-3 py-1.5 rounded-full border transition-all ${darkMode ? "border-gray-600 text-gray-400 hover:border-purple-500 hover:text-purple-400" : "border-gray-300 text-gray-600 hover:border-purple-500 hover:text-purple-600"}`}>
                    {s}
                  </button>
                ))}
              </div>
              ...
              <div className={`p-4 border-t ${darkMode ? "border-gray-700" : "border-gray-200"}`}>
                <div className="flex gap-2">
                  <input
                    ...
                    className={`flex-1 px-4 py-2.5 border rounded-xl text-sm focus:outline-none focus:border-purple-500 ${darkMode ? "bg-gray-800 border-gray-700 text-white placeholder-gray-500" : "bg-white border-gray-300 text-gray-800 placeholder-gray-400"}`}
                  />
                  <button onClick={() => sendMessage()} className="px-5 py-2.5 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-sm font-medium">
                    {t.send}
                  </button>
                </div>
          ...
          {/* TICKETS TAB - Mobile friendly table */}
          {activeTab === "tickets" && (
                      ...
                      {ticketsData.map((t) => (
                        <tr key={t.id} className={`border-b ${darkMode ? "border-gray-700" : "border-gray-100"}`}>
                          ...
                          <td className="px-4 py-3">
                            <span className={`text-xs px-2 py-1 rounded-full ${t.status === "Open" ? "bg-red-100 text-red-600 dark:bg-red-500/20 dark:text-red-400" : t.status === "Resolved" ? "bg-green-100 text-green-600 dark:bg-green-500/20 dark:text-green-400" : "bg-yellow-100 text-yellow-600 dark:bg-yellow-500/20 dark:text-yellow-400"}`}>
                              {t.status}
                            </span>
                           </td>
                          <td className="px-4 py-3">
                            <span className={`text-xs px-2 py-1 rounded-full ${t.priority === "High" ? "bg-purple-100 text-purple-600 dark:bg-purple-500/20 dark:text-purple-400" : "bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400"}`}>
                              {t.priority}
                            </span>
                           </td>
                         </tr>
              ...
              <div className="max-w-md mx-auto space-y-4">
                <div className={`p-5 rounded-xl border ${darkMode ? "border-gray-700 bg-gray-800/50" : "border-gray-200 bg-white"}`}>
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className={`text-xs font-medium ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{t.username}</label>
                      <input defaultValue="user_demo" className={`w-full mt-1 px-3 py-2 text-sm border rounded-lg focus:outline-none focus:border-purple-500 ${darkMode ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-300 text-gray-800"}`} />
                    </div>
                    <div>
                      <label className={`text-xs font-medium ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{t.email}</label>
                      <input defaultValue="user@example.com" className={`w-full mt-1 px-3 py-2 text-sm border rounded-lg focus:outline-none focus:border-purple-500 ${darkMode ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-300 text-gray-800"}`} />
                    </div>
                  </div>
                </div>
                <button className="w-full py-3 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-sm font-medium transition-all">
                  {t.saveChanges}
                </button>
              </div>
            </div>
