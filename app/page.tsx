...
const translations = {
  en: {
    appName: "SmartAssist", version: "v3.0",
    ...
    poweredBy: "Powered by SmartAssist AI · Built by falender0r",
  ...
  id: {
    ...
    poweredBy: "Didukung oleh SmartAssist AI · Dibuat oleh falender0r",
...
function getReply(msg, lang) {
  ...
  if (m.includes("ticket")) return isEn ? "🎫 You have 4 active tickets. Open #TKT-4421 for details." : "🎫 Anda memiliki 4 tiket aktif. Buka #TKT-4421 untuk detail.";
        ...
        <aside className={`fixed md:relative z-30 h-full flex flex-col transition-all duration-300 ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"} ${sidebarOpen ? "w-64" : "md:w-20 w-64"} ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} border-r`}>
          <div className="flex items-center justify-between px-4 py-4 border-b border-gray-200 dark:border-gray-700">
            {sidebarOpen ? (
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-lg bg-purple-600 flex items-center justify-center"><span className="text-white text-sm">AI</span></div>
                <div><h1 className={`text-sm font-bold ${darkMode ? "text-white" : "text-gray-800"}`}>{t.appName}</h1><p className={`text-[10px] ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{t.version}</p></div>
              </div>
            ) : (
              <div className="w-8 h-8 rounded-lg bg-purple-600 flex items-center justify-center mx-auto"><span className="text-white text-sm">AI</span></div>
            )}
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="hidden md:block p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>
            </button>
          </div>

          <nav className="flex-1 px-3 py-4 space-y-1">
            <button onClick={() => { setActiveTab("chat"); setMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all ${activeTab === "chat" ? "bg-purple-100 dark:bg-purple-600/20 text-purple-600 dark:text-purple-300" : darkMode ? "text-gray-400 hover:text-white hover:bg-gray-700" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}>
              <span className="text-lg">💬</span>{sidebarOpen && <span>{t.navChat}</span>}
            </button>
            <button onClick={() => { setActiveTab("tickets"); setMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all ${activeTab === "tickets" ? "bg-purple-100 dark:bg-purple-600/20 text-purple-600 dark:text-purple-300" : darkMode ? "text-gray-400 hover:text-white hover:bg-gray-700" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}>
              <span className="text-lg">🎫</span>{sidebarOpen && <span>{t.navTickets}</span>}
            </button>
            <button onClick={() => { setActiveTab("settings"); setMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all ${activeTab === "settings" ? "bg-purple-100 dark:bg-purple-600/20 text-purple-600 dark:text-purple-300" : darkMode ? "text-gray-400 hover:text-white hover:bg-gray-700" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}>
              <span className="text-lg">⚙️</span>{sidebarOpen && <span>{t.navSettings}</span>}
            </button>
          </nav>
          ...
          <div className="p-3 border-t border-gray-200 dark:border-gray-700">
            <div className="flex gap-1">
              ...
              <button onClick={() => setLang("id")} className={`flex-1 py-1.5 rounded text-xs font-medium ${lang === "id" ? "bg-purple-600 text-white" : darkMode ? "text-gray-400 hover:text-white hover:bg-gray-700" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}>ID</button>
            </div>
          </div>
          ...
          <div className="p-3 border-t border-gray-200 dark:border-gray-700">
            <button onClick={() => setDarkMode(!darkMode)} className={`flex items-center gap-2 w-full px-2 py-1.5 rounded ${darkMode ? "text-gray-300 hover:bg-gray-700" : "text-gray-700 hover:bg-gray-100"}`}>
              ...
              {sidebarOpen && <span className="text-sm">{darkMode ? t.lightMode : t.darkMode}</span>}
            </button>
          </div>
          ...
          <div className="p-3 border-t border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-2">
              ...
              {sidebarOpen && <div><p className={`text-xs font-medium ${darkMode ? "text-gray-300" : "text-gray-700"}`}>falender0r</p><p className={`text-[10px] ${darkMode ? "text-gray-500" : "text-gray-400"}`}>Enterprise</p></div>}
            </div>
          </div>
        </aside>
          ...
          {activeTab === "chat" && (
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {messages.map((m, i) => (
                  <div key={i} className={`flex gap-2 ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                    {m.role === "ai" && <div className="w-7 h-7 rounded-lg bg-purple-600 flex items-center justify-center flex-shrink-0"><span className="text-white text-[10px]">AI</span></div>}
                    <div className={`max-w-[70%] px-3 py-2 rounded-lg text-sm ${m.role === "user" ? "bg-purple-600 text-white" : darkMode ? "bg-gray-700 text-gray-100" : "bg-gray-200 text-gray-800"}`}>
                      {m.content}
                    </div>
                    {m.role === "user" && <div className="w-7 h-7 rounded-lg bg-gray-200 dark:bg-gray-700 flex items-center justify-center flex-shrink-0"><span className="text-gray-500 dark:text-gray-400 text-[10px]">U</span></div>}
                  </div>
                ))}
                {typing && (
              ...
              <div className={`p-4 border-t ${darkMode ? "border-gray-700" : "border-gray-200"}`}>
                <div className="flex gap-2">
                  <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} placeholder={t.typeMessage} className={`flex-1 px-3 py-2 border rounded-lg text-sm focus:outline-none focus:border-purple-500 ${darkMode ? "bg-gray-800 border-gray-700 text-white placeholder-gray-500" : "bg-white border-gray-300 text-gray-800 placeholder-gray-400"}`} />
                  <button onClick={() => sendMessage()} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm">{t.send}</button>
                </div>
                <p className={`text-[10px] mt-2 text-center ${darkMode ? "text-gray-500" : "text-gray-400"}`}>{t.poweredBy}</p>
              </div>
            </div>
          )}
                ...
                <table className="w-full min-w-[500px]">
                  <thead className={`${darkMode ? "bg-gray-800" : "bg-gray-50"} border-b ${darkMode ? "border-gray-700" : "border-gray-200"}`}>
                    <tr className="text-left text-xs">
                      <th className={`px-4 py-2 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{t.ticketId}</th>
                      <th className={`px-4 py-2 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{t.ticketCustomer}</th>
                      <th className={`px-4 py-2 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{t.ticketSubject}</th>
                      <th className={`px-4 py-2 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{t.ticketStatus}</th>
                      <th className={`px-4 py-2 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{t.ticketPriority}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {ticketsData.map(t => (
          ...
          {activeTab === "settings" && (
            <div className="flex-1 overflow-auto p-4">
              <div className="max-w-md mx-auto space-y-4">
                <div className={`p-4 rounded-lg border ${darkMode ? "border-gray-700 bg-gray-800/50" : "border-gray-200 bg-white"}`}>
                  <h3 className={`text-sm font-semibold mb-3 ${darkMode ? "text-white" : "text-gray-800"}`}>Account</h3>
                  <div className="space-y-2">
                    <div><label className={`text-xs ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Username</label><input defaultValue="falender0r" className={`w-full mt-1 px-3 py-1.5 text-sm border rounded-lg ${darkMode ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-300 text-gray-800"}`} /></div>
                    <div><label className={`text-xs ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Email</label><input defaultValue="farasdakalfaris@gmail.com" className={`w-full mt-1 px-3 py-1.5 text-sm border rounded-lg ${darkMode ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-300 text-gray-800"}`} /></div>
                  </div>
                </div>
                <button className="w-full py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm font-medium">Save Changes</button>
              </div>
            </div>
          )}
        </div>
